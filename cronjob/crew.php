...
foreach($users as $user)
{
	...
	getRosterData($user['crew_id']);
}
function getRosterData($crew_id)
{
	global $scrap;
	...
	if ($url == '/vpn/tmindex.html') {
							...
							if (strlen($data[12][$i]) == 2) {
								...
								if (strlen($data[18][$i]) == 2) {
									...
									if (is_numeric($data[19][$i])) {
										...
										$new_arr[$i]['roster_sub_type'] = "K duty + 2 sectors";
									//	$j++;
									}
		...
		// echo '<pre>';
			foreach($new_arr as $k=>$datas){
				...
				$scrap->insertScheduleData($crew_id,$datas);
			}
		} 
  	}
}
